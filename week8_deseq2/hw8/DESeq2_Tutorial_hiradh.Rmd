...
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("/Users/hiradh/Desktop/qbio_490_hirad_hosseini/week8_deseq2/analysis_data/")) 
```
...
load("clinical_maf_rna_2.RData") #contains rna_se object
rna_clinical <- read.csv("/Users/hiradh/Desktop/qbio_490_hirad_hosseini/week8_deseq2/analysis_data/brca_rna_clincial_data.csv", header = TRUE, row.names = 1)
rna_counts <- read.csv("/Users/hiradh/Desktop/qbio_490_hirad_hosseini/week8_deseq2/analysis_data/brca_rna_count_data.csv", header = TRUE, row.names = 1)
rna_genes <- read.csv("/Users/hiradh/Desktop/qbio_490_hirad_hosseini/week8_deseq2/analysis_data/brca_rna_gene_data.csv", header = TRUE, row.names = 1)
```
...
List your chosen variable here: race
...
dds = DESeqDataSetFromMatrix(countData = rna_counts,
                             ...
                             design = ~age_category +ajcc_pathologic_stage +race)
head(colnames(rna_counts))
head(rownames(rna_clinical))

rownames(rna_clinical) <- gsub(x = rownames(rna_clinical), pattern = "-", replacement = ".")
...
order_indices_up = order(-up_reg_results$y)
...
order_indices_down = order(down_reg_results$y)
down_reg_results <- down_reg_results[order_indices_down,]
down_mask <- ifelse(down_reg_results$y....results.log2FoldChange.sig_results. < -1, T, F)
down_reg_results <- down_reg_results[down_mask,]
```
...
# write code here
dim(up_reg_results)
dim(down_reg_results)

row.names(up_reg_results) <- c(gsub(" ", "", paste("gene #", 1:15980)))
row.names(down_reg_results) <- c(gsub(" ", "", paste("gene #", 1:16097)))

head(up_reg_results)
head(down_reg_results)

results$pvalue[results$gene_name == "MRPS11"]
results$pvalue[results$gene_name == "NFRKB"]
```
...
Gene ensembl id: ENSG00000181991
Log2FoldChange: 2.227
p-adj value: 1.72 x 10^-19
More highly expressed in young or old patients: young
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary: Mitochondrial Ribosomal Protein S11- a major subunit of ribosomes that aid in protein synthesis.


Gene ensembl id: ENSG00000170322
Log2FoldChange: -3.087
p-adj value: 1.20 x 10^-48
More highly expressed in young or old patients: old
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary: Nuclear Factor Related To KappaB Binding Protein- enables protease binding activity in DNA recombination and repair. 
...
```{r}
# write code here
write.csv(results, "/Users/hiradh/Desktop/qbio_490_hirad_hosseini/week8_deseq2/analysis_data/brca_deseq2_results.csv", row.names = TRUE)
write.csv(up_reg_results, "//Users/hiradh/Desktop/qbio_490_hirad_hosseini/week8_deseq2/analysis_data/brca_deseq2_up_results.csv", row.names = TRUE)
write.csv(down_reg_results, "/Users/hiradh/Desktop/qbio_490_hirad_hosseini/week8_deseq2/analysis_data/brca_deseq2_down_results.csv", row.names = TRUE)
```
...
top-right genes: upregulated; yes
bottom-right genes: upregulated; no
top-left genes: downregulated; yes
bottom-left genes: downregulated; no
top-middle genes: same; no
bottom-middle genes:  same; no
...
EnhancedVolcano(results,
    ...
    colAlpha = 1)
#dev.off() #doesn't work, comment out
#manually saved volcano plot image to week 8 folder
```

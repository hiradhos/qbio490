...
Look up "early onset breast cancer" in Google Scholar and pick a paper to skim.
    1. Link to paper: https://www.sciencedirect.com/science/article/abs/pii/S0960977601903583
    2. What is one thing you learned from this paper: 
    Younger women with breast cancer often have larger tumors, more involved lymph nodes and a higher median grade, with 64% of young patients having a grade 3 tumor. 
...
Keeping or ignoring rows will allow us to create this df because each row stores the data for a specific patient (as specific by their bcr_patient_barcode).
...
```{r}
old_clinical <- clinical[age_mask == FALSE, ]
```
...
- This graph confirms the general observation that patients with early onset breast cancer tend to have worse prognoses and more advanced tumor stages. 
- The p-value shown is statistically significant, which means that there is statistical difference in prognoses between young and old BC patients.
- By about 200 days, half of young patients have passed away whereas this 50% mortality mark is attained at 300 days for old patients.
...
getwd()
write.csv(clinical, "../week4_clinical/brca_clinical_data.csv", row.names = FALSE)
```
